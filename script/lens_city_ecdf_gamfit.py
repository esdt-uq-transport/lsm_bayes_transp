# Selected "city" empirical, and parametric distributions/locations for LENS
...
qfnm = 'LENS_NAmer_H2OSNO_%d_Quantile_ParamFit.nc' % (tgtdt)
...
h2oqsgam = ncclm.variables['H2OSNO_quantile_gamfit'][:,:,:]
...
for k in range(ncity):
    pyplot.text(ctyfrm['Longitude'].values[k], ctyfrm['Latitude'].values[k], ctyfrm['Label'].values[k], \
                fontsize=20, color=c6[k], verticalalignment='center', transform=trnsfrm)
...
for k in range(ncity):
    ...
    plst.append(pl1)
    pl2, = p7.plot(h2oqsgam[yspt,xspt,:],prbs,'--',c=c6[k],linewidth=2)

    # Quick summary frame
    mxfrm = pandas.DataFrame({'Prob': prbs[196:207], 'SWEQuant': h2oqs[yspt,xspt,196:207]})
    print(mxfrm)

p7.set_ylim(-0.05,1.05)
p7.set_xlim(-30,550)
p7.set_xlabel('SWE [mm]',size=14)
p7.set_ylabel('CDF',size=14)
