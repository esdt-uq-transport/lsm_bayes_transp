...
# Output figures for SWE manuscript
...
for j in range(nvld):
    ...
    medout[yidx[j],xidx[j]] = numpy.median(ytmp)
prpout = ma.masked_where(prpout < -9000,prpout)
medout = ma.masked_where(medout < -9000,medout)
...
pltnm = 'LENS_H2OSNO_BTMNonzeroProp_NAmer_%d.pdf' % (tgtdt)
pyplot.tight_layout()
pyplot.savefig(pltnm)
pyplot.close()
...
trnsfrm = ccrs.PlateCarree()
pmp = fig.add_subplot(1,1,1, projection=ccrs.LambertConformal(central_longitude=-105, central_latitude=33) )
cs = pyplot.pcolormesh(lnarr,ltarr,medout,vmin=0,vmax=500,transform = trnsfrm,cmap=pyplot.get_cmap('Blues'))
pmp.coastlines(color='#777777',linewidth=0.5)
pmp.set_extent([-140, -70, 10, 75])
cbar = pyplot.colorbar(cs,extend='max',orientation='horizontal',shrink=0.5,pad=0.06)
cbar.set_label('SWE [mm]',size=14)
cbar.ax.tick_params(labelsize=12)
tstr = 'BTM Ensemble Median SWE %d' % (tgtdt) 
pyplot.title(tstr,size=16)

pltnm = 'LENS_H2OSNO_BTMMedianSWE_NAmer_%d.pdf' % (tgtdt)
pyplot.tight_layout()
pyplot.savefig(pltnm)
pyplot.close()
